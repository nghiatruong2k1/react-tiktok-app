...
function CardVideoHeaderComponent({ loading, name, description, avatar, id }) {
    return (
        <CardHeader
            ...
            avatar={
                <Avatar className={'fullview'} src={avatar} alt={name ?? ''}>
            ...
            title={<>{loading ? <Skeleton className="fullview" /> : name}</>}
            subheader={<>{loading ? <Skeleton className="fullview" /> : description}</>}
            action={
                <LoadingButton loading={loading} variant="outlined" color="error">
                    Theo dõi
                </LoadingButton>
            }
        />
