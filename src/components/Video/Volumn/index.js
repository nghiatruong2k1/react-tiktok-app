...
function VolumnButtonComponent(props){
    ...
    return (
        <span 
            onMouseEnter={(e)=>{if(!isOpen){open()}}} 
            onMouseLeave={(e)=>{if(isOpen){close()}}}
             
        >
            <IconButton onClick={()=>{toggleMutex()}} ref={ref} size='small'>
                {
                    (numberVol === 0 || isMutex) ? <IconVolumeOff /> 
                    : ((numberVol > 40) ? <IconVolume /> : <IconVolume2 />)
                }
            </IconButton>
            <Popper  
                ...
                >
                    <Stack alignItems={'center'} justifyContent={'center'} height={'100%'}>
                        <Slider  
                            ...
                            value={numberVol}
                            onChange={(event, newValue) => {setVol(newValue)}}
